...
const Certificates = () => {
  ...
  const fetchCerts = async () => {
    try {
      const res = await fetch("https://codealpha-portfolio-1.onrender.comapi/certificates");
  ...
  const handleSubmit = async (e) => {
    ...
    const url = editingId ? `https://codealpha-portfolio-1.onrender.comapi/certificates/${editingId}` : "https://codealpha-portfolio-1.onrender.comapi/certificates";
  ...
  const handleDelete = async (id) => {
    if(window.confirm("Are you sure you want to delete this?")) {
      try {
        const res = await fetch(`https://codealpha-portfolio-1.onrender.comapi/certificates/${id}`, { method: 'DELETE' });
        const data = await res.json();
