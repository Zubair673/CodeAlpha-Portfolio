...
const Messages = () => {
  ...
  const fetchMessages = async () => {
    try {
      const res = await fetch("https://codealpha-portfolio-drii.onrender.com/api/messages");
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this message?")) {
      try {
        const res = await fetch(`https://codealpha-portfolio-drii.onrender.com/api/messages/${id}`, { method: 'DELETE' });
        const data = await res.json();
        if (data.success) {
